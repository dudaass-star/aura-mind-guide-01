...
- **Twilio 63027 nos avisos 1 e 2**: o template `HXaf4af...` tem botão `https://olaaura.com.br/pagamento?t={{2}}`, então `{{2}}` deve ser SÓ o token. O código passava a URL completa → `/pagamento?t=https://...` → 63027 e nenhum aviso era entregue. Corrigido em `_shared/dunning-whatsapp.ts`.
