...
function Cta({ source, label, children, className = "" }: { source: "hero" | "pricing" | "sticky" | "header" | "final" | "demo"; label: string; children: ReactNode; className?: string }) {
  return (
    <Button asChild variant="sage" size="lg" className={className}>
      <Link to={`${checkoutHref(source, V)}&plan=essencial&billing=monthly`} onClick={() => trackLandingCta(source, label, V)}>{children}</Link>
    </Button>
  );
}
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-10 px-5 pb-14 sm:px-8 lg:min-h-[720px] lg:grid-cols-[1.08fr_.92fr] lg:gap-16 lg:pb-14">
        <div className="v4-rise relative z-10 max-w-3xl">
          ...
          <div className="mt-8 flex flex-col items-start gap-3 sm:flex-row sm:items-center">
            ...
            <p className="text-xs leading-relaxed text-primary-foreground/55">7 dias por R$ 6,90<br className="hidden sm:block" /> · depois R$ 29,90/mês</p>
          </div>
...
function AppScreenHeader({ eyebrow, title, icon: Icon }: { eyebrow: string; title: string; icon: typeof Sparkles }) {
  return <div className="flex items-center gap-3 border-b border-border bg-card/90 px-4 py-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-secondary text-primary"><Icon className="h-5 w-5" /></span><div><p className="text-[9px] font-bold uppercase text-muted-foreground">{eyebrow}</p><p className="font-display text-base font-semibold">{title}</p></div></div>;
}

function JourneyValueScreen() {
  return <div className="overflow-hidden rounded-2xl border bg-background text-foreground shadow-card"><AppScreenHeader eyebrow="Conteúdos para você" title="Jornadas" icon={BookOpen} /><div className="space-y-4 p-4"><section className="rounded-xl border bg-card p-4"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-secondary text-primary"><BookOpen className="h-4 w-4" /></span><div><p className="text-[10px] font-bold uppercase text-primary">Em andamento</p><h4 className="mt-1 font-display text-lg font-semibold">Limites sem culpa</h4><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Para entender o desconforto de se escolher sem confundir limite com abandono.</p></div></div><div className="mt-4 flex items-center justify-between text-[10px] font-semibold text-muted-foreground"><span>2 de 6 concluídos</span><span>33%</span></div><div className="mt-2 h-1.5 rounded-full bg-secondary"><div className="h-full w-1/3 rounded-full bg-primary" /></div></section><div><p className="text-[10px] font-bold uppercase text-primary">Sua sequência</p><div className="mt-2 flex items-center gap-3 rounded-xl border bg-card p-3"><span className="flex h-10 w-10 items-center justify-center rounded-lg bg-secondary text-sm font-bold text-primary">3</span><div className="min-w-0 flex-1"><p className="text-sm font-bold">O desconforto de se escolher</p><p className="text-xs text-muted-foreground">Novo · mais recente</p></div><ArrowRight className="h-4 w-4 text-muted-foreground" /></div></div></div></div>;
}

function MeditationValueScreen() {
  return <div className="overflow-hidden rounded-2xl border bg-background text-foreground shadow-card"><AppScreenHeader eyebrow="Sua pausa" title="Meditações" icon={Headphones} /><div className="space-y-4 p-4"><p className="text-[10px] font-bold uppercase text-primary">Sugerida pra você</p><section className="rounded-xl border bg-card p-4"><div className="flex items-start gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-accent text-accent-foreground"><Headphones className="h-4 w-4" /></span><div><h4 className="font-display text-lg font-semibold">Antes de uma decisão difícil</h4><p className="mt-1 text-xs font-semibold text-muted-foreground">8 min · Clareza</p></div></div><p className="mt-3 text-xs leading-relaxed text-muted-foreground">Uma pausa guiada para baixar o ruído e voltar ao que realmente importa antes de escolher.</p><div className="mt-4 flex items-center gap-3 rounded-xl bg-secondary p-2.5"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary text-primary-foreground"><Play className="h-4 w-4 fill-current" /></span><div className="flex-1"><div className="h-1 rounded-full bg-background"><div className="h-full w-2/5 rounded-full bg-primary" /></div><p className="mt-1 text-[9px] text-muted-foreground">3:12 / 8:00</p></div><span className="text-[10px] font-bold text-primary">1x</span></div></section><p className="text-xs leading-relaxed text-muted-foreground">Não aparece como resposta automática. É um recurso disponível quando uma pausa pode ajudar.</p></div></div>;
}

function PortraitValueScreen() {
  return <div className="overflow-hidden rounded-2xl border bg-background text-foreground shadow-card"><AppScreenHeader eyebrow="Sua história" title="Sobre você" icon={UserRound} /><div className="space-y-4 p-4"><section className="rounded-xl border border-primary/20 bg-secondary/60 p-4"><p className="text-[10px] font-bold uppercase text-primary">Seu retrato atual</p><h4 className="mt-1 font-display text-lg font-semibold">O que estamos construindo juntos</h4><p className="mt-2 text-xs leading-relaxed text-muted-foreground">O que você contou aparece como fato. O que a AURA percebeu aparece como leitura para você confirmar.</p></section><section className="rounded-xl border bg-card p-4"><div className="mb-2 flex items-center gap-2 text-primary"><Sparkles className="h-4 w-4" /><p className="text-[10px] font-bold uppercase">Leitura para confirmar</p></div><p className="text-sm leading-relaxed">Talvez você esteja aprendendo a diferenciar responsabilidade de carregar tudo sozinho.</p><p className="mt-3 text-[10px] text-muted-foreground">Percebido pela AURA — ainda não confirmado</p><div className="mt-2 flex gap-2"><span className="rounded-md border px-3 py-2 text-[11px] font-bold"><Check className="mr-1 inline h-3 w-3" /> Faz sentido</span><span className="px-2 py-2 text-[11px] font-bold text-muted-foreground">Não foi bem assim</span></div></section></div></div>;
}

function ProgressValueScreen() {
  return <div className="overflow-hidden rounded-2xl border bg-background text-foreground shadow-card"><AppScreenHeader eyebrow="Sua evolução" title="Percurso" icon={Sparkles} /><div className="space-y-4 p-4"><section><p className="text-[10px] font-bold uppercase text-primary">Sua história em movimento</p><h4 className="mt-1 font-display text-xl font-semibold">Marina, sua história já está ganhando forma.</h4><p className="mt-2 text-xs leading-relaxed text-muted-foreground">O que antes eram conversas soltas começa a revelar continuidade — sempre com a sua confirmação.</p><div className="mt-4 grid grid-cols-3 divide-x divide-border rounded-xl border bg-card py-3 text-center"><div><strong className="block font-display text-lg">2</strong><span className="text-[9px] text-muted-foreground">temas presentes</span></div><div><strong className="block font-display text-lg">5</strong><span className="text-[9px] text-muted-foreground">movimentos</span></div><div><strong className="block font-display text-lg">3</strong><span className="text-[9px] text-muted-foreground">capítulos</span></div></div></section><section className="rounded-xl border bg-card p-4"><div className="flex items-center gap-2 text-primary"><Quote className="h-4 w-4" /><p className="text-[10px] font-bold uppercase">Movimento recente</p></div><p className="mt-2 text-sm leading-relaxed">Talvez exista um movimento aqui: você começou a sustentar um limite sem precisar justificar cada detalhe.</p><p className="mt-3 flex items-center gap-1 text-[10px] font-bold text-primary"><Check className="h-3 w-3" /> Confirmado por você</p></section></div></div>;
}

const connectedFeatures = [
  { id: "jornadas", eyebrow: "Continuar", title: "Uma descoberta ganha tempo para amadurecer.", text: "Jornadas aprofundam um tema em episódios curtos, liberados no seu ritmo — sem transformar reflexão em tarefa acumulada.", screen: <JourneyValueScreen /> },
  { id: "meditacoes", eyebrow: "Atravessar o momento", title: "Nem todo momento pede mais palavras.", text: "Meditações guiadas oferecem uma pausa quando você precisa diminuir o ruído, regular o ritmo ou permanecer com algo que apareceu.", screen: <MeditationValueScreen /> },
  { id: "sobre-voce", eyebrow: "Reconhecer", title: "A AURA aprende sem tomar posse da sua história.", text: "Em Sobre você, fatos e leituras ficam visíveis. Você confirma o que representa você, corrige o que não ficou certo ou apaga.", screen: <PortraitValueScreen /> },
  { id: "percurso", eyebrow: "Perceber o caminho", title: "Mudanças que passariam despercebidas começam a aparecer.", text: "O Percurso conecta temas, movimentos, sessões e capítulos para você reconhecer o que vem mudando — sem inventar progresso antes da hora.", screen: <ProgressValueScreen /> },
] as const;

export function ConnectedAppValueV4() {
  const [active, setActive] = useState(0);
  const feature = connectedFeatures[active];
  return <section id="app-conectado" className="bg-background py-20 sm:py-28"><div className="mx-auto max-w-6xl px-5 sm:px-8"><div className="mx-auto max-w-4xl text-center"><p className="text-xs font-bold uppercase text-primary">Tudo se conecta ao que você está vivendo</p><h2 className="v4-balance mt-4 font-display text-4xl font-semibold leading-tight sm:text-6xl">O que você vive não fica perdido em conversas soltas.</h2><p className="mx-auto mt-6 max-w-3xl leading-relaxed text-muted-foreground">A AURA conecta conversas, sessões e descobertas para aprofundar o que importa e tornar visível o caminho que você está construindo.</p></div><div className="mt-12 grid gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-center lg:gap-20"><div><div className="grid grid-cols-2 gap-2">{connectedFeatures.map((item, index) => <Button key={item.id} type="button" variant={active === index ? "default" : "outline"} className="h-auto min-h-12 justify-start whitespace-normal px-3 py-2 text-left" onClick={() => setActive(index)}>{item.id === "jornadas" ? <BookOpen /> : item.id === "meditacoes" ? <Headphones /> : item.id === "sobre-voce" ? <UserRound /> : <BarChart3 />}{item.id === "sobre-voce" ? "Sobre você" : item.id.charAt(0).toUpperCase() + item.id.slice(1)}</Button>)}</div><p className="mt-8 text-xs font-bold uppercase text-primary">{feature.eyebrow}</p><h3 className="v4-balance mt-3 font-display text-3xl font-semibold leading-tight sm:text-4xl">{feature.title}</h3><p className="mt-4 max-w-lg leading-relaxed text-muted-foreground">{feature.text}</p><div className="mt-6 border-l-2 border-primary pl-4 text-sm font-semibold">Conversa, sessão e descoberta passam a fazer parte da mesma história.</div></div><div className="mx-auto w-full max-w-[430px]">{feature.screen}</div></div><div className="mt-12 rounded-xl bg-foreground px-6 py-7 text-background sm:px-8"><div className="flex items-center gap-5"><div><p className="text-xs font-bold uppercase text-accent">Uma experiência, não seis ferramentas</p><p className="mt-2 max-w-3xl font-display text-xl font-semibold leading-snug sm:text-2xl">Você conversa. A AURA aprofunda, conecta e devolve um caminho que continua depois que a tela fecha.</p></div><BarChart3 className="hidden h-9 w-9 shrink-0 text-accent sm:block" /></div></div></div></section>;
}
...
export function HowItWorksV4() {
  const steps = [
    ...
    { n: "03", title: "O App transforma percepção em continuidade", text: "Hoje orienta o próximo passo, Jornadas aprofundam temas, Meditações ajudam a atravessar momentos e Sobre você e Percurso preservam o que faz sentido." },
  ];
...
export function SocialProofV4() {
  return <section aria-label="Avaliações reais" className="v4-ink-soft py-14 sm:py-16"><div className="mx-auto max-w-6xl px-5 sm:px-8"><div className="grid gap-8 lg:grid-cols-[1fr_1.15fr] lg:items-center"><div><p className="text-xs font-bold uppercase text-primary">Experiência avaliada por quem já viveu uma sessão</p><h2 className="v4-balance mt-3 font-display text-3xl font-semibold leading-tight text-primary-foreground sm:text-4xl">Não precisa confiar só no que a página promete.</h2><p className="mt-4 max-w-lg text-sm leading-relaxed text-primary-foreground/65">As notas abaixo vêm das avaliações registradas ao final de sessões reais com a AURA.</p></div><div className="grid grid-cols-3 divide-x divide-primary-foreground/15 border-y border-primary-foreground/15 py-5 text-center"><div className="px-2"><strong className="block font-display text-3xl text-primary-foreground">4,6/5</strong><span className="mt-1 block text-[11px] text-primary-foreground/55">média das sessões</span></div><div className="px-2"><strong className="block font-display text-3xl text-primary-foreground">166</strong><span className="mt-1 block text-[11px] text-primary-foreground/55">avaliações recebidas</span></div><div className="px-2"><strong className="block font-display text-3xl text-primary-foreground">125</strong><span className="mt-1 block text-[11px] text-primary-foreground/55">notas máximas</span></div></div></div></div></section>;
}
...
const plans: { id: PlanId; name: string; sessions: string; description: string; trialPrice: string; primary?: boolean }[] = [
  ...
  { id: "transformacao", name: "Transformação", sessions: "8 encontros por mês", description: "Para fases que pedem presença e acompanhamento mais frequentes.", trialPrice: "R$ 19,90" },
];

export function PricingV4() {
  const [showAll, setShowAll] = useState(false);
  const visible = showAll ? plans : plans.filter((plan) => plan.primary);
  return <section id="precos" className="bg-background py-20 sm:py-28"><div className="mx-auto max-w-6xl px-5 sm:px-8"><div className="mx-auto max-w-3xl text-center"><p className="text-xs font-bold uppercase text-primary">A forma mais simples de começar</p><h2 className="v4-balance mt-4 font-display text-4xl font-semibold leading-tight sm:text-5xl">Viva a experiência completa da AURA por sete dias.</h2><p className="mt-5 text-muted-foreground">Converse por texto ou áudio, explore todo o App e viva sua primeira sessão guiada por R$ 6,90. Depois, continue no Essencial por R$ 29,90 por mês.</p></div><div className={`mx-auto mt-12 grid max-w-5xl gap-5 ${showAll ? "md:grid-cols-3" : "max-w-md"}`}>{visible.map((plan) => <article key={plan.id} className={`flex flex-col rounded-2xl border bg-card p-6 ${plan.primary ? "border-primary shadow-card" : "border-border"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase text-primary">{plan.primary ? "Comece por aqui" : "Mais encontros"}</p><h3 className="mt-2 font-display text-3xl font-semibold">{plan.name}</h3></div>{plan.primary && <Sparkles className="text-primary" />}</div><p className="mt-4 text-sm leading-relaxed text-muted-foreground">{plan.description}</p><div className="mt-6 border-y border-border py-5"><p className="font-display text-3xl font-semibold">{plan.trialPrice}<span className="font-body text-sm font-normal text-muted-foreground"> / 7 dias</span></p><p className="mt-1 text-xs text-muted-foreground">depois {fmtBRL(PLAN_MONTHLY_EQUIVALENT[plan.id].monthly)}/mês</p></div><ul className="my-6 flex-1 space-y-3 text-sm"><li className="flex gap-2"><Check className="text-primary" /> Conversas por texto e áudio quando precisar</li><li className="flex gap-2"><Check className="text-primary" /> {plan.sessions} de 45 minutos</li><li className="flex gap-2"><Check className="text-primary" /> Jornadas e Meditações no seu ritmo</li><li className="flex gap-2"><Check className="text-primary" /> Sobre você e Percurso conectados à sua história</li></ul><Button asChild variant={plan.primary ? "sage" : "outline"} size="lg" className="w-full"><Link to={`${checkoutHref("pricing", V)}&plan=${plan.id}&billing=monthly`} onClick={() => trackLandingCta("pricing", `${plan.name} mensal (v4)`, V)}>Experimentar por {plan.trialPrice}</Link></Button></article>)}</div><div className="mt-6 text-center"><Button type="button" variant="ghost" onClick={() => setShowAll((value) => !value)}>{showAll ? <><ChevronUp /> Voltar para o Essencial</> : <><ChevronDown /> Ver planos com mais sessões</>}</Button><p className="mt-4 text-xs text-muted-foreground">Cartão ou PIX · cancelamento simples · outras periodicidades disponíveis no checkout</p></div></div></section>;
}
...
const faqs = [
  ...
  ["Como funcionam os 7 dias iniciais?", "Você começa no Essencial e paga R$ 6,90 para experimentar por sete dias. Depois, o plano continua por R$ 29,90 por mês. Antes do pagamento, o checkout mostra essas condições com clareza. Você pode cancelar."],
