...
export function ConnectedAppValueV4() {
  return <section id="app-conectado" className="bg-background py-20 sm:py-28"><div className="mx-auto max-w-6xl px-5 sm:px-8"><div className="mx-auto max-w-4xl text-center"><p className="text-xs font-bold uppercase text-primary">Tudo se conecta ao que você está vivendo</p><h2 className="v4-balance mt-4 font-display text-4xl font-semibold leading-tight sm:text-6xl">O que você vive não fica perdido em conversas soltas.</h2><p className="mx-auto mt-6 max-w-3xl leading-relaxed text-muted-foreground">A AURA conecta conversas, sessões e descobertas para ajudar você a aprofundar o que importa, cuidar do momento presente e enxergar o caminho que está construindo.</p></div><div className="mt-16 space-y-20 sm:space-y-28">{connectedFeatures.map((feature, index) => <article key={feature.id} className="grid items-center gap-10 lg:grid-cols-2 lg:gap-20"><div className={index % 2 ? "lg:order-2" : ""}><p className="text-xs font-bold uppercase text-primary">{String(index + 3).padStart(2, "0")} · {feature.eyebrow}</p><h3 className="v4-balance mt-4 font-display text-3xl font-semibold leading-tight sm:text-4xl">{feature.title}</h3><p className="mt-5 max-w-lg leading-relaxed text-muted-foreground">{feature.text}</p><div className="mt-6 flex items-center gap-3 border-l-2 border-primary pl-4 text-sm font-semibold"><ArrowRight className="h-4 w-4 shrink-0 text-primary" /><span>{index === 0 ? "A conversa continua em um tema." : index === 1 ? "O tema encontra uma pausa possível." : index === 2 ? "A percepção só vira referência com você." : "As partes começam a formar uma história."}</span></div></div><div className={`mx-auto w-full max-w-[430px] ${index % 2 ? "lg:order-1" : ""}`}>{feature.screen}</div></article>)}</div><div className="mt-20 rounded-2xl bg-foreground px-6 py-8 text-background sm:px-10"><div className="grid gap-6 sm:grid-cols-[1fr_auto] sm:items-center"><div><p className="text-xs font-bold uppercase text-accent">Uma experiência, não seis ferramentas</p><p className="mt-2 max-w-3xl font-display text-2xl font-semibold leading-snug sm:text-3xl">Você conversa. A AURA aprofunda, conecta e devolve um caminho que continua fazendo sentido depois que a tela fecha.</p></div><BarChart3 className="h-10 w-10 text-accent" /></div></div></div></section>;
}
