...
function monthLabel(key: string) {
  return new Date(`${key}-15T12:00:00-03:00`).toLocaleDateString("pt-BR", { month: "long", year: "numeric", timeZone: "America/Sao_Paulo" });
}
...
function formatSessionDate(value: string) {
  return new Date(value).toLocaleString("pt-BR", { weekday: "long", day: "2-digit", month: "long", hour: "2-digit", minute: "2-digit", timeZone: "America/Sao_Paulo" });
}
  ...
  return (
    <div className="portal-area-page space-y-6">
      <section className="flex items-center justify-between border-b border-border pb-4">
        ...
        <div className="text-center"><p className="text-[10px] font-bold uppercase text-muted-foreground">Sua agenda</p><h2 className="mt-1 font-display text-lg font-semibold capitalize">{monthLabel(selectedMonth)}</h2></div>
          ...
          {upcomingSessions.map((session, index) => {
            ...
            return (
              <article key={session.id} className={index === 0 ? "rounded-lg bg-foreground p-5 text-background shadow-card" : "rounded-lg border border-border bg-card p-4 shadow-sm"}>
                ...
                <p className={`mt-2 font-semibold capitalize leading-snug ${index === 0 ? "text-xl" : "text-base"}`}>{formatSessionDate(session.scheduled_at)}</p>
                {canEnter ? (
      ...
      <Dialog open={schedulerOpen} onOpenChange={(open) => !saving && setSchedulerOpen(open)}><DialogContent className="w-[calc(100%-2rem)] max-w-sm rounded-lg"><DialogHeader className="text-left"><DialogTitle>{editing ? "Escolher um novo horário" : "Agendar sessão"}</DialogTitle><DialogDescription>Horários de Brasília em {monthLabel(selectedMonth)}. Opções próximas de outra sessão já ficam ocultas.</DialogDescription></DialogHeader><div className="space-y-4 py-2"><label className="block text-sm font-medium">Dia<Input className="mt-1.5" type="date" min={minDate} max={maxDate} value={date} onChange={(event) => { setDate(event.target.value); setTime(""); }} /></label><label className="block text-sm font-medium">Horário<select className="mt-1.5 flex h-10 w-full rounded-md border border-input bg-background px-3 text-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring md:text-sm" value={time} onChange={(event) => setTime(event.target.value)}><option value="">Selecione</option>{availableTimes.map((slot) => <option key={slot} value={slot}>{slot}</option>)}</select></label>{availableTimes.length === 0 && <p className="text-sm text-muted-foreground">Não há horários disponíveis neste dia.</p>}</div><DialogFooter><Button type="button" variant="outline" onClick={() => setSchedulerOpen(false)} disabled={saving}>Voltar</Button><Button type="button" onClick={() => void manageSession(editing ? "reschedule" : "schedule")} disabled={!date || !time || saving}>{saving ? "Confirmando..." : "Confirmar horário"}</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={preparationOpen} onOpenChange={(open) => !saving && setPreparationOpen(open)}><DialogContent className="w-[calc(100%-2rem)] max-w-sm rounded-lg"><DialogHeader className="text-left"><DialogTitle>Preparar este encontro</DialogTitle><DialogDescription>Opcional. O que você não quer esquecer de levar para esta sessão?</DialogDescription></DialogHeader><Textarea value={preparation} onChange={(event) => setPreparation(event.target.value)} maxLength={500} rows={5} placeholder="Pode ser uma situação, dúvida ou algo que ficou da última conversa." /><p className="text-right text-xs text-muted-foreground">{preparation.length}/500</p><DialogFooter><Button type="button" variant="outline" onClick={() => setPreparationOpen(false)}>Voltar</Button><Button type="button" disabled={saving || !selectedSession} onClick={() => selectedSession && void saveExperience("save_preparation", selectedSession, preparation)}>{saving ? "Guardando..." : "Guardar preparação"}</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={correctionOpen} onOpenChange={(open) => !saving && setCorrectionOpen(open)}><DialogContent className="w-[calc(100%-2rem)] max-w-sm rounded-lg"><DialogHeader className="text-left"><DialogTitle>Ajustar esta leitura</DialogTitle><DialogDescription>Conte o que não representa bem o que você viveu. A AURA usa seu ajuste nas próximas conversas.</DialogDescription></DialogHeader><Textarea value={correction} onChange={(event) => setCorrection(event.target.value)} maxLength={800} rows={5} placeholder="O que seria mais fiel para você?" /><p className="text-right text-xs text-muted-foreground">{correction.length}/800</p><DialogFooter><Button type="button" variant="outline" onClick={() => setCorrectionOpen(false)}>Voltar</Button><Button type="button" disabled={saving || !selectedSession || !correction.trim()} onClick={() => selectedSession && void saveExperience("correct_reframe", selectedSession, correction)}>{saving ? "Guardando..." : "Guardar ajuste"}</Button></DialogFooter></DialogContent></Dialog>

      <AlertDialog open={cancelOpen} onOpenChange={(open) => !saving && setCancelOpen(open)}><AlertDialogContent className="w-[calc(100%-2rem)] max-w-sm rounded-lg"><AlertDialogHeader><AlertDialogTitle>Cancelar esta sessão?</AlertDialogTitle><AlertDialogDescription>Ela deixa de ocupar uma sessão do mês e você poderá escolher outro horário depois.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={saving}>Manter sessão</AlertDialogCancel><AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={(event) => { event.preventDefault(); void manageSession("cancel"); }} disabled={saving}>{saving ? "Cancelando..." : "Cancelar sessão"}</AlertDialogAction></AlertDialogFooter></AlertDialog>
    </div>
  );
}