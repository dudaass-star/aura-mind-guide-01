...
import { ArrowLeft, MessageSquare, Send, Loader2, AlertTriangle, RefreshCw, Search } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b sticky top-0 z-10 bg-background/95 backdrop-blur">
        <div className="container mx-auto p-4 flex items-center gap-3">
          ...
          <Button variant="ghost" size="sm" onClick={fetchList} className="ml-auto">
            <RefreshCw className="h-4 w-4 mr-1" /> Atualizar
          </Button>
        </div>
      </header>

      <Tabs defaultValue="recuperacao" className="container mx-auto p-4">
        <TabsList>
          <TabsTrigger value="recuperacao">Recuperação ({conversations.length})</TabsTrigger>
          <TabsTrigger value="oficial">Número Oficial</TabsTrigger>
        </TabsList>

        <TabsContent value="oficial" className="mt-4">
          <Card className="p-6 text-center">
            <p className="text-sm text-muted-foreground mb-4">
              O painel completo de conversas com usuários da Aura (número oficial) está em outra página.
            </p>
            <Button onClick={() => navigate('/admin/mensagens')}>
              Abrir painel oficial
            </Button>
          </Card>
        </TabsContent>

        <TabsContent value="recuperacao" className="mt-4">
