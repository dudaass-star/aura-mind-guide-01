...
export default function AdminMessages() {
  ...
  useEffect(() => {
    ...
    if (statusFilter !== 'all') {
      filtered = filtered.filter(u => {
        ...
        if (statusFilter === 'cancelled') return u.status === 'cancelled' || u.status === 'paused';
        return true;
      });
    }
  ...
  const isUnread = (user: UserWithMessages) =>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              <span className={`text-sm truncate ${unread ? 'font-bold text-foreground' : 'font-medium text-foreground'}`}>
                                ...
                                <TooltipTrigger asChild>
                                  <div>
                                    <Badge variant="secondary" className="text-[10px] px-1.5 py-0 flex-shrink-0">
                                      {user.month_message_count}
                                    </Badge>
                                  </div>
                                </TooltipTrigger>
                                <TooltipContent side="left">
                                  <p className="text-xs">Mês: {user.month_message_count} · Total: {user.message_count}</p>
                                </TooltipContent>
                              </Tooltip>
                            </div>
                            <div className="flex items-center gap-1.5 mt-1">
                              ...
                              <span className="text-[10px] text-muted-foreground">
                                mês: {user.month_message_count} msgs
                              </span>
                            </div>
                          </div>
                        </button>
