...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // ========== TRIAL UNIFICADO (Subscription nativa com add_invoice_items) ==========
      // Novo fluxo CIT→MIT: a Subscription já vem criada pelo Checkout com trial 7d
      // e o item avulso do trial (R$ 6,90/9,90/19,90) cobrado no 1º invoice.
      // Aqui só fazemos o onboarding (profile, welcome, CAPI, GA4) — não criamos sub manual.
      if (session.metadata?.trial_unified === 'true' && session.mode === 'subscription') {
        console.log('🆕 Trial UNIFIED flow detected (CIT→MIT continuous mandate)');
        ...
        if (!customerPhone) {
          console.error('❌ No phone in trial_unified session');
        ...
        try {
          console.log('✅ Trial unified subscription:', subscriptionId, '— skipping manual creation');
          ...
          try {
            if (!existingProfile) {
              ...
              console.log('✅ Trial-unified profile created');
            } else {
              ...
              console.log('✅ Trial-unified profile updated');
            }
          } catch (dbError) {
            console.error('❌ Database error (trial_unified):', dbError);
          ...
          await sendGa4Purchase({
            ...
            email: customerEmail || undefined,
            transactionId: `${session.id}_trial_unified`,
            value: (session.amount_total || 0) / 100,
            plan: customerPlan,
            planName: planName,
          ...
          console.error('❌ Error in trial_unified flow:', unifiedError);
          return new Response(JSON.stringify({ received: true }), {
            status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
