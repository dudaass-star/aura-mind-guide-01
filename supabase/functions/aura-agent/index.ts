...
interface PhaseEvaluation {
  guidance: string | null;    // Instrução a injetar no dynamicContext (null = sem intervenção)
  detectedPhase: string;      // Fase detectada pelo conteúdo das mensagens
  stagnationLevel: number;    // 0 = ok, 1 = alerta leve, 2 = intervenção forte
}

// Palavras-chave que indicam cada fase terapêutica no conteúdo da Aura
...
function evaluateTherapeuticPhase(
  ...
  sessionActive: boolean,
  sessionPhase?: string,   // phase from calculateSessionTimeContext
  sessionElapsedMin?: number
): PhaseEvaluation {
  // Get last N assistant messages to analyze Aura's current behavior
  ...
  // Count phase indicators in recent responses
  ...
  // Determine detected phase by dominant indicators
  ...
  // Count how many questions Aura asked in last messages (sign of exploration loop)
  const questionCount = recentAssistant.reduce((sum, msg) => 
    sum + (msg.match(/\?/g) || []).length, 0
  );

  // Count user-assistant exchange pairs
  const recentPairs = Math.floor(
    messageHistory.filter(m => m.role === 'user').slice(-10).length
  );

  // ======== SESSION MODE ========
  if (sessionActive && sessionPhase && sessionElapsedMin !== undefined) {
    // In sessions, phases are time-driven. Detect CONTENT vs TIME mismatch.
    
    if (['reframe', 'development', 'transition'].includes(sessionPhase)) {
      // Time says reframe/later, but content is still exploration
...
"Então, com base nisso que a gente explorou... o que faria sentido como próximo passo pra você?"`
    ...
    // Early session: still in opening after 8+ minutes
    if (sessionPhase === 'exploration' && sessionElapsedMin! > 8 && detectedPhase === 'presenca') {
      const isStillAsking = questionCount > 4;
      if (isStillAsking) {
        return {
          detectedPhase: 'presenca',
          stagnationLevel: 1,
          guidance: `\n\n🔄 AVALIAÇÃO DE FASE:
Já passou da abertura (${sessionElapsedMin} min). Você está fazendo muitas perguntas exploratórias sem aprofundar.
AÇÃO: Escolha O tema principal que surgiu e vá fundo nele. Use investigação socrática.
"De tudo que você trouxe, o que mais tá pesando? Vamos focar nisso."`
  ...
  // Only intervene if there's emotional depth (not ping-pong)
  ...
  // In deep mode: check stagnation after 5+ exchanges
...
- ESCOLHA UMA. Não faça checklist.`
  ...
  if (recentPairs >= 8 && detectedPhase === 'sentido') {
    return {
      ...
      stagnationLevel: 1,
      guidance: `\n\n🔄 AVALIAÇÃO DE FASE (CONVERSA PROFUNDA):
O usuário já explorou o sentido por ${recentPairs}+ trocas. Se já emergiu clareza, conduza para MOVIMENTO (Fase 3).

AÇÃO:
- "Com tudo isso que a gente explorou... o que o menor passo em direção a isso pareceria pra você?"
- Se o sentido ainda não apareceu, tudo bem continuar — mas mude o ângulo da pergunta.`
    };
  }
