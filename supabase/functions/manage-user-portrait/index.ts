  ...
  try {
    ...
    if (body.action === "add_fact") {
      const { data, error } = await admin.from("user_insights").insert({
        user_id: userId, category: "user_added", key: CATEGORY_LABELS[body.category], value: body.value,
    ...
    if (body.action === "edit_fact" || body.action === "delete_fact") {
      const { data: insight } = await admin.from("user_insights").select("id,key,value").eq("id", body.insightId).eq("user_id", userId).eq("category", "user_added").maybeSingle();
