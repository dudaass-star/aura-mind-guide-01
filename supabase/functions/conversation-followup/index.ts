...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      // Buscar todos os profiles ativos com telefone
      const { data: activeProfiles, error: profilesError } = await supabase
        .from('profiles')
        .select('user_id, name, phone, plan, current_session_id, do_not_disturb_until, whatsapp_instance_id, current_journey_id, current_episode')
        .eq('status', 'active')
        .not('phone', 'is', null);

      if (profilesError) {
        console.error('❌ Error fetching active profiles for re-engagement:', profilesError);
      } else {
        console.log(`📋 Found ${activeProfiles?.length || 0} active profiles to check for re-engagement`);
        ...
        for (const profile of activeProfiles || []) {
          try {
            ...
            const daysSinceLastMsg = (now - new Date(lastUserMsg.created_at).getTime()) / (1000 * 60 * 60 * 24);
            ...
            // Check if user received a message in last 10 minutes (avoid overlap)
            const { data: recentAssistantMsg } = await supabase
              .from('messages')
              .select('created_at')
              .eq('user_id', profile.user_id)
              .eq('role', 'assistant')
            ...
            if (recentAssistantMsg) {
              const minsSinceLastAssistant = (now - new Date(recentAssistantMsg.created_at).getTime()) / 60000;
              if (minsSinceLastAssistant < 10) continue;
            }

            console.log(`🎯 Re-engagement candidate: ${profile.user_id} (${profile.name}), inactive ${Math.round(daysSinceLastMsg)}d`);

            // Gather context for AI message generation
            ...
            if (profile.current_journey_id) {
              const { data: journey } = await supabase
                ...
                .maybeSingle();
              if (journey) {
                journeyInfo = `Jornada atual: "${journey.title}" (episódio ${profile.current_episode || 0}/${journey.total_episodes})`;
              }
            }

            const conversationSnippet = (recentMsgs || [])
              .slice(0, 8)
              .reverse()
              .map((m: any) => `${m.role === 'user' ? 'Usuário' : 'AURA'}: ${m.content.substring(0, 200)}`)
              .join('\n');

            const insightsText = (insights || [])
              .map((i: any) => `${i.key}: ${i.value}`)
              .join('; ');
                ...
                const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
                  ...
                  body: JSON.stringify({
                    ...
                    messages: [
                      {
                        role: 'system',
                        content: `Você é a AURA, uma amiga próxima que entende de psicologia. O usuário "${profile.name || 'querido(a)'}" está inativo há ${Math.round(daysSinceLastMsg)} dias.

${journeyInfo ? journeyInfo : 'Sem jornada ativa.'}
${insightsText ? `Insights sobre o usuário: ${insightsText}` : ''}

Últimas mensagens da conversa:
${conversationSnippet || '(sem histórico recente)'}

IMPORTANTE:
- Se o contexto indicar situação muito sensível (crise, luto, trauma), retorne SKIP
- Caso contrário, gere UMA mensagem curta (1-2 frases, máx 120 chars) que:
  - Faça referência ESPECÍFICA a algo da vida do usuário ou da jornada
  - Seja acolhedora mas NÃO genérica (nada de "como você está?" ou "tudo bem?")
  - Use linguagem informal brasileira
  - Use no máximo 1 emoji
  - Mostre que lembrou do usuário de forma genuína`
                      },
                      { role: 'user', content: 'Gere a mensagem de reengajamento:' }
                    ],
                    max_tokens: 80,
                  }),
                });
                ...
                console.error('⚠️ AI re-engagement generation error:', err);
              }
            }

            // Skip if AI decided to skip or failed
            if (!message || message.toUpperCase() === 'SKIP') {
              console.log(`⚠️ Re-engagement skipped for ${profile.user_id}: ${message ? 'sensitive context' : 'AI failed'}`);
              continue;
            }
            ...
            if (sendResult.success) {
              console.log(`✅ Re-engagement sent to ${profile.name} (${profile.user_id}): "${message}"`);
              reengagementSent++;

              // Upsert conversation_followups record
              await supabase
                .from('conversation_followups')
                .upsert({
              ...
              console.error(`❌ Re-engagement send failed for ${profile.user_id}: ${sendResult.error}`);
            }
          } catch (userErr) {
            console.error(`❌ Re-engagement error for ${profile.user_id}:`, userErr);
          }
        }
      }
    } catch (reengageErr) {
