CREATE OR REPLACE FUNCTION public.manage_portal_session(
  _action text,
...
BEGIN
  ...
  IF _action NOT IN ('schedule', 'reschedule', 'cancel') THEN
  ...
  IF _action = 'schedule' THEN
    IF _scheduled_at IS NULL OR _scheduled_at <= _now THEN
      RAISE EXCEPTION 'future_time_required' USING ERRCODE = '22023';
    END IF;

    IF EXISTS (
      SELECT 1 FROM public.sessions
      WHERE user_id = _user_id AND status IN ('scheduled', 'in_progress')
    ) THEN
    ...
    WHERE user_id = _user_id
      AND status IN ('scheduled', 'in_progress', 'completed', 'no_show')
      AND scheduled_at >= _month_start
      AND scheduled_at < _next_month;

    IF _limit <= 0 OR _used >= _limit THEN
    ...
    INSERT INTO public.sessions (
      ...
      _user_id, _scheduled_at, 'scheduled', 'livre', 45, 'portal',
      false, false, false, NULL, false, true
    ) RETURNING * INTO _session;

  ELSIF _action = 'reschedule' THEN
    IF _session_id IS NULL OR _scheduled_at IS NULL OR _scheduled_at <= _now THEN
      RAISE EXCEPTION 'future_time_required' USING ERRCODE = '22023';
    END IF;

    SELECT * INTO _session
    FROM public.sessions
    WHERE id = _session_id AND user_id = _user_id AND status = 'scheduled'
    FOR UPDATE;
    ...
    IF _session.scheduled_at < _now THEN
  ...
  ELSE
    ...
    UPDATE public.sessions
    SET status = 'cancelled', waiting_for_scheduled_time = false
    WHERE id = _session_id
    RETURNING * INTO _session;
  END IF;
...
REVOKE ALL ON FUNCTION public.manage_portal_session(text, timestamptz, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.manage_portal_session(text, timestamptz, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.manage_portal_session(text, timestamptz, uuid) TO service_role;