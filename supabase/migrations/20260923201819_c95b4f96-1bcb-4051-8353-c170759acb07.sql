...
BEGIN
  ...
  IF _action IN ('start', 'switch') THEN
    ...
    IF v_profile.current_journey_id IS NOT NULL AND v_profile.current_journey_id <> _journey_id THEN
      UPDATE public.user_journey_history SET status = 'switched', ended_at = now() WHERE user_id = _user_id AND journey_id = v_profile.current_journey_id AND status = 'active';
    END IF;
    ...
    INSERT INTO public.user_journey_history (user_id, journey_id, started_at, status, origin) VALUES (_user_id, _journey_id, now(), 'active', CASE WHEN _action = 'start' THEN 'guided_choice' ELSE 'switch' END) ON CONFLICT DO NOTHING;
    ...
    IF FOUND THEN
      ...
      UPDATE public.profiles SET current_episode = 1, last_content_sent_at = now() WHERE user_id = _user_id;
    END IF;
    RETURN jsonb_build_object('status', 'active', 'journey_id', _journey_id, 'episode_id', v_episode.id);
  ELSIF _action = 'pause' THEN UPDATE public.profiles SET journey_paused = true WHERE user_id = _user_id; RETURN jsonb_build_object('status', 'paused');
  ELSIF _action = 'resume' THEN UPDATE public.profiles SET journey_paused = false WHERE user_id = _user_id; RETURN jsonb_build_object('status', 'active'); END IF;
  IF _episode_id IS NULL THEN RAISE EXCEPTION 'episode_required'; END IF;
  SELECT * INTO v_episode FROM public.journey_episodes WHERE id = _episode_id; IF NOT FOUND THEN RAISE EXCEPTION 'episode_not_found'; END IF;
  SELECT * INTO v_progress FROM public.journey_episode_progress WHERE user_id = _user_id AND episode_id = _episode_id FOR UPDATE; IF NOT FOUND THEN RAISE EXCEPTION 'episode_not_released'; END IF;
  IF _action = 'open' THEN
    IF v_progress.status <> 'completed' THEN UPDATE public.journey_episode_progress SET status = 'in_progress', opened_at = COALESCE(opened_at, now()), updated_at = now() WHERE id = v_progress.id; END IF;
  ...
  ELSIF _action = 'progress' THEN
    IF v_progress.status <> 'completed' THEN UPDATE public.journey_episode_progress SET status = 'in_progress', opened_at = COALESCE(opened_at, now()), progress_percent = GREATEST(progress_percent, LEAST(99, GREATEST(1, COALESCE(_progress_percent, 1)))), updated_at = now() WHERE id = v_progress.id; END IF;
    RETURN jsonb_build_object('status', CASE WHEN v_progress.status = 'completed' THEN 'completed' ELSE 'in_progress' END);
  ELSIF _action = 'reflect' THEN
    ...
    UPDATE public.journey_episode_progress SET reflection_text = trim(_reflection_text), reflection_saved_at = now(), updated_at = now() WHERE id = v_progress.id; RETURN jsonb_build_object('status', 'reflected');
  ELSIF _action = 'discuss' THEN UPDATE public.journey_episode_progress SET discussed_at = COALESCE(discussed_at, now()), updated_at = now() WHERE id = v_progress.id; RETURN jsonb_build_object('status', 'discussed');
  ELSIF _action = 'complete' THEN
    ...
    IF v_episode.episode_number >= v_total THEN
      UPDATE public.user_journey_history SET status = 'completed', ended_at = now(), final_episode_id = _episode_id WHERE user_id = _user_id AND journey_id = v_episode.journey_id AND status = 'active';
