...
BEGIN
  ...
  WHERE user_id = _user_id AND status IN ('released', 'in_progress')
  ...
  FROM public.journey_episodes
  WHERE journey_id = v_profile.current_journey_id
    AND episode_number = v_profile.current_episode + 1;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('released', false, 'reason', 'journey_finished');
  END IF;
